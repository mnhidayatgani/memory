...
- [x] T001 Create project directory structure per plan.md (src/hmc/, tests/{contract,integration,unit}/, examples/) <!-- Implements FR-001 -->
- [x] T002 Create pyproject.toml with all dependencies (chromadb>=0.4.0, typer>=0.9.0, pytest>=7.4.0, mypy>=1.5.0, black>=23.0.0, ruff>=0.1.0) <!-- Implements FR-001, NFR-016 -->
- [x] T003 Configure project.scripts entry point: `hmc = "hmc.cli:app"` in pyproject.toml <!-- Implements FR-007 -->
- [x] T004 Create .gitignore (ignore .hmc_memory/, venv/, **pycache**/, .mypy_cache/, .pytest_cache/, htmlcov/, dist/, build/)
- [x] T005 Create README.md with quickstart examples for greenfield and brownfield use cases <!-- Implements NFR-013 -->
- [x] T006 Create LICENSE file (MIT or Apache 2.0)
- [x] T007 [P] Configure pytest in pyproject.toml (testpaths, coverage target 85%, markers for slow/integration/contract tests) <!-- Implements NFR-011 -->
- [x] T008 [P] Configure mypy in pyproject.toml (strict mode, Python 3.10+, disallow_untyped_defs=true) <!-- Implements FR-021, NFR-012 -->
...
- [x] T011 Create src/hmc/py.typed marker file for PEP 561 type stub support <!-- Implements FR-021 -->
- [x] T012 Create src/hmc/**init**.py with package exports (HybridMemoryCore, exceptions) <!-- Implements FR-001 -->
...
- [x] T013 Create src/hmc/exceptions.py with custom exception hierarchy (HMCError, StorageError, ValidationError, SeederError) including Google-style docstrings <!-- Implements FR-020, FR-022, NFR-014 -->
- [x] T014 Create src/hmc/interfaces.py with FactualStorage ABC (setup, set_fact, get_fact methods) including full type hints and Google-style docstrings <!-- Implements FR-005, FR-020, FR-021 -->
- [x] T015 Create src/hmc/interfaces.py with SemanticStorage ABC (setup, add_semantic, query_semantic methods) including full type hints and Google-style docstrings <!-- Implements FR-005, FR-020, FR-021 -->
- [x] T016 Add abstractmethod decorators and proper ABC inheritance to all interface methods in src/hmc/interfaces.py <!-- Implements FR-005, FR-006 -->
...
- [x] T019 [P] [US1] Create src/hmc/backends.py with SQLiteFactualStore class skeleton (inherits FactualStorage) <!-- Implements FR-003, FR-005 -->
- [x] T020 [US1] Implement SQLiteFactualStore.**init**(db_path) with lazy connection initialization in src/hmc/backends.py <!-- Implements FR-003 -->
- [x] T021 [US1] Implement SQLiteFactualStore.\_get_connection() method with error handling in src/hmc/backends.py <!-- Implements FR-003, NFR-014 -->
- [x] T022 [US1] Implement SQLiteFactualStore.setup() to create facts table with schema from contracts/database-schema.sql <!-- Implements FR-003, NFR-010 -->
- [x] T023 [US1] Implement SQLiteFactualStore.set_fact(key, value) with JSON serialization and INSERT OR REPLACE in src/hmc/backends.py <!-- Implements FR-003, NFR-006, NFR-010 -->
- [x] T024 [US1] Implement SQLiteFactualStore.get_fact(key) with JSON deserialization and None handling in src/hmc/backends.py <!-- Implements FR-003, NFR-002 -->
- [x] T025 [US1] Add input validation to SQLiteFactualStore methods (non-empty key, JSON-serializable value) raising ValidationError <!-- Implements FR-022, NFR-008 -->
- [x] T026 [P] [US1] Create src/hmc/backends.py with ChromaSemanticStore class skeleton (inherits SemanticStorage) <!-- Implements FR-004, FR-005 -->
- [x] T027 [US1] Implement ChromaSemanticStore.**init**(collection_name, persist_directory) with lazy client initialization in src/hmc/backends.py <!-- Implements FR-004, FR-014 -->
- [x] T028 [US1] Implement ChromaSemanticStore.\_get_client() to initialize chromadb.PersistentClient in src/hmc/backends.py <!-- Implements FR-004, FR-014, NFR-006 -->
- [x] T029 [US1] Implement ChromaSemanticStore.\_get_collection() to get_or_create_collection in src/hmc/backends.py <!-- Implements FR-004 -->
- [x] T030 [US1] Implement ChromaSemanticStore.setup() for idempotent collection creation in src/hmc/backends.py <!-- Implements FR-019, NFR-007 -->
- [x] T031 [US1] Implement ChromaSemanticStore.add_semantic(content, metadata) with UUID generation and collection.add() in src/hmc/backends.py <!-- Implements FR-004, FR-012 -->
- [x] T032 [US1] Implement ChromaSemanticStore.query_semantic(query_text, k, filter) with collection.query() and result transformation in src/hmc/backends.py <!-- Implements FR-013, NFR-002 -->
- [x] T033 [US1] Add input validation to ChromaSemanticStore methods (non-empty content, k > 0, metadata dict with string keys) raising ValidationError <!-- Implements FR-022, NFR-008 -->

### Core API for User Story 1

- [x] T034 [US1] Create src/hmc/core.py with HybridMemoryCore class skeleton <!-- Implements FR-002 -->
- [x] T035 [US1] Implement HybridMemoryCore.**init**(project_id, db_directory, factual_store, semantic_store) with dependency injection support <!-- Implements FR-002, FR-006 -->
- [x] T036 [US1] Add project_id validation (alphanumeric + hyphens/underscores) and db_directory creation in HybridMemoryCore.**init** <!-- Implements FR-022, NFR-008 -->
- [x] T037 [US1] Initialize default backends (SQLiteFactualStore, ChromaSemanticStore) if not provided in HybridMemoryCore.**init** <!-- Implements FR-003, FR-004 -->
- [x] T038 [US1] Call setup() on both storage backends in HybridMemoryCore.**init** <!-- Implements FR-015, NFR-007 -->
- [x] T039 [US1] Implement HybridMemoryCore.set_fact(key, value) delegating to factual store in src/hmc/core.py <!-- Implements FR-002 -->
- [x] T040 [US1] Implement HybridMemoryCore.get_fact(key) delegating to factual store in src/hmc/core.py <!-- Implements FR-002 -->
- [x] T041 [US1] Implement HybridMemoryCore.add_semantic(content, metadata) delegating to semantic store in src/hmc/core.py <!-- Implements FR-002, FR-012 -->
- [x] T042 [US1] Implement HybridMemoryCore.query_semantic(query_text, k, filter) delegating to semantic store in src/hmc/core.py <!-- Implements FR-002, FR-013 -->
- [x] T043 [US1] Add comprehensive Google-style docstrings to all HybridMemoryCore public methods with Args/Returns/Raises sections <!-- Implements FR-020 -->

### Seeder Utility for User Story 1 (Core brownfield feature)

- [x] T044 [P] [US1] Create src/hmc/chunker.py with chunk_file_content(content, max_size=2000) function implementing paragraph-aware splitting <!-- Implements FR-011, NFR-005 -->
- [x] T045 [US1] Implement line range tracking in chunker.py for each chunk (returns list[dict] with 'content' and 'line_range') <!-- Implements FR-012 -->
- [x] T046 [US1] Add logic to split oversized sections by character boundaries in chunker.py <!-- Implements FR-011, NFR-005 -->
- [x] T047 [P] [US1] Create src/hmc/seeder.py with seed_project(project_path, memory_dir, project_id, verbose) main function skeleton <!-- Implements FR-007 -->
- [x] T048 [US1] Implement input validation in seed_project (path exists, is directory, readable) raising SeederError <!-- Implements FR-022, NFR-008, NFR-009 -->
- [x] T049 [US1] Implement project_id sanitization (regex to replace invalid chars) in seed_project <!-- Implements FR-007, NFR-008 -->
- [x] T050 [US1] Initialize HybridMemoryCore and store seeding metadata (**project_root**, **seeded_at**, **hmc_version**) in seed_project <!-- Implements FR-007, FR-014 -->
- [x] T051 [US1] Create parse_persona_md(content) function in src/hmc/seeder.py to extract factual and semantic sections using regex <!-- Implements FR-008 -->
- [x] T052 [US1] Implement Factual section parsing in parse_persona_md (regex: `## Factual` followed by `- key: value` lines) <!-- Implements FR-008, FR-016 -->
- [x] T053 [US1] Implement Semantic section parsing in parse_persona_md (regex: `## Semantic` followed by `- "example"` lines) <!-- Implements FR-008, FR-017 -->
- [x] T054 [US1] Add persona seeding logic in seed*project: check for persona.md, parse it, store facts with \_\_persona* prefix, embed voice examples with type="persona_voice" <!-- Implements FR-008, FR-016, FR-017 -->
- [x] T055 [US1] Add graceful error handling for malformed persona.md (log warning, continue with valid sections) <!-- Implements NFR-014 -->
- [x] T056 [US1] Create extract_tech_stack(project_path) function in src/hmc/seeder.py <!-- Implements FR-009 -->
- [x] T057 [US1] Implement requirements.txt parsing in extract_tech_stack (split lines, skip comments) <!-- Implements FR-009 -->
...
- [x] T065 [US1] Implement source file globbing in seed*project (patterns: *.py, _.js, _.ts, \_.md)
...
**Status**: 29 CLI tests pending (Typer framework testing complexity)
...
2. **Current Validation**:
   ...
   - ✅ Manual testing confirms all CLI features operational

3. **Pending Tests** (tests/unit/test_cli.py):
   - Command argument parsing validation
   - Option flag handling (--memory-dir, --project-id, --verbose)
...
**Reference**: See tests/integration/test_seeder_brownfield.py for comprehensive CLI validation through actual usage patterns.
