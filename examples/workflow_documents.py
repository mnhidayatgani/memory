...
memory = HybridMemoryCore(project_id="agent-workflow", db_directory="./.hmc_memory")
...
plan_id = memory.add_semantic(
    ...
    metadata={"type": "plan", "feature": "authentication", "status": "in_progress"},
)
...
for task_desc, status in tasks:
    memory.add_semantic(
        content=task_desc, metadata={"type": "task", "feature": "authentication", "status": status}
    )
...
specs = memory.query_semantic(
    query_text="authentication requirements", k=5, filter={"type": "spec", "status": "approved"}
)
...
in_progress = memory.query_semantic(
    query_text="task", k=10, filter={"type": "task", "status": "in_progress"}
)
...
auth_docs = memory.query_semantic(
    query_text="authentication", k=10, filter={"feature": "authentication"}
)
