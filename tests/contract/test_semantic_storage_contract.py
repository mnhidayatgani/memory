...
class TestSemanticStorageContract:
    ...
    def test_query_semantic_filter_parameter(self, store):
        ...
        store.add_semantic(
            content="Spec document for login",
            metadata={"type": "spec", "feature": "login"}
        )
        store.add_semantic(
            content="Plan document for login",
            metadata={"type": "plan", "feature": "login"}
        )
        store.add_semantic(
            content="Spec document for payment",
            metadata={"type": "spec", "feature": "payment"}
        )
    ...
    def test_add_semantic_validates_metadata_keys(self, store):
        ...
        with pytest.raises(ValidationError, match="string"):
            store.add_semantic(
                content="test",
                metadata={123: "value"}  # Non-string key
            )
    ...
    def test_similarity_ranking(self, store):
        ...
        store.add_semantic(
            content="Java programming language guide",
