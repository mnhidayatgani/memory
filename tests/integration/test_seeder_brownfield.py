...
"""
    ...
    def test_seed_complete_project(self, sample_project):
        ...
        assert (memory_dir / "facts.db").exists()
    ...
    def test_persona_facts_stored(self, sample_project):
        ...
        hmc = HybridMemoryCore(
            project_id="test-persona", db_directory=sample_project / ".hmc_memory"
        )
    ...
    def test_persona_voices_embedded(self, sample_project):
        ...
        hmc = HybridMemoryCore(
            project_id="test-voices", db_directory=sample_project / ".hmc_memory"
        )

        # Query for voice examples
        results = hmc.query_semantic("test suite analysis", k=5, filter={"type": "persona_voice"})
    ...
    def test_tech_stack_extracted(self, sample_project):
        ...
        hmc = HybridMemoryCore(project_id="test-tech", db_directory=sample_project / ".hmc_memory")

        tech_stack = hmc.get_fact("__tech_stack__")
        assert tech_stack is not None
        assert isinstance(tech_stack, dict)

        # Should have dependencies from requirements.txt
        deps = tech_stack.get("dependencies", [])
        assert any("pytest" in d for d in deps)
        assert any("black" in d for d in deps)
        assert any("mypy" in d for d in deps)
    ...
    def test_project_structure_stored(self, sample_project):
        ...
        hmc = HybridMemoryCore(
            project_id="test-structure", db_directory=sample_project / ".hmc_memory"
        )
        ...
        # Should have found src directory
        assert "src" in structure["directories"]

        # Should have found source files
    ...
    def test_code_chunks_embedded(self, sample_project):
        ...
        hmc = HybridMemoryCore(project_id="test-code", db_directory=sample_project / ".hmc_memory")

        # Search for main function
        results = hmc.query_semantic("main entry point", k=5, filter={"type": "code_chunk"})
    ...
    def test_semantic_search_for_code(self, sample_project):
        ...
        hmc = HybridMemoryCore(
            project_id="test-search", db_directory=sample_project / ".hmc_memory"
        )

        # Search for helper function
        results = hmc.query_semantic(
            "helper function add numbers", k=3, filter={"type": "code_chunk"}
        )
    ...
    def test_multiple_seeding_runs(self, sample_project):
        ...
        hmc = HybridMemoryCore(project_id="test-multi", db_directory=sample_project / ".hmc_memory")
        assert hmc.get_fact("__persona_name__") == "TestBot"
    ...
    def test_metadata_facts_stored(self, sample_project):
        ...
        hmc = HybridMemoryCore(
            project_id="test-metadata", db_directory=sample_project / ".hmc_memory"
        )
