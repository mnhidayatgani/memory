...
class TestPersistence:
    ...
    def test_semantic_data_persists_across_sessions(self, temp_dir):
        ...
        doc_id = hmc1.add_semantic(
            "Persistent semantic document", metadata={"type": "test", "persistent": True}
        )
    ...
    def test_mixed_data_persists(self, temp_dir):
        ...
        hmc1.add_semantic(
            "I always remember our conversations.", metadata={"type": "persona_voice"}
        )
        hmc1.add_semantic("Your data is safe with me.", metadata={"type": "persona_voice"})
        ...
        results = hmc2.query_semantic(
            "remember conversations", k=5, filter={"type": "persona_voice"}
        )
        assert len(results) > 0
    ...
    def test_large_dataset_persistence(self, temp_dir):
        ...
        for i in range(50):
            hmc1.add_semantic(
                f"Document number {i} about topic {i % 5}", metadata={"index": i, "topic": i % 5}
            )
        ...
        for topic in range(5):
            results = hmc2.query_semantic(f"topic {topic}", k=20, filter={"topic": topic})
    ...
    def test_empty_query_persistence(self, temp_dir):
        ...
        HybridMemoryCore(project_id=project_id, db_directory=temp_dir)
