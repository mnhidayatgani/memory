...
class TestChromaSemanticStore:
    ...
    def test_init_stores_collection_name(self, temp_dir):
        ...
        store = ChromaSemanticStore(collection_name="my_collection", persist_directory=temp_dir)
    ...
    def test_init_stores_persist_directory(self, temp_dir):
        ...
        store = ChromaSemanticStore(collection_name="test", persist_directory=temp_dir / "chroma")
    ...
    def test_init_client_is_none(self, temp_dir):
        ...
        store = ChromaSemanticStore(collection_name="test", persist_directory=temp_dir)
    ...
    def test_add_semantic_generates_id(self, store):
        ...
        doc_id = store.add_semantic(content="Test document", metadata={"type": "test"})
    ...
    def test_add_semantic_stores_metadata(self, store):
        ...
        store.add_semantic(content="Tutorial document", metadata=metadata)
    ...
    def test_query_semantic_filters_by_metadata(self, store):
        ...
        results = store.query_semantic(query_text="doc", k=10, filter={"type": "spec"})
    ...
    def test_semantic_similarity_ranking(self, store):
        ...
        store.add_semantic(
            "Complete guide to Python programming fundamentals", metadata={"relevance": "high"}
        )
        store.add_semantic("JavaScript and Python comparison", metadata={"relevance": "medium"})
        store.add_semantic("How to cook pasta", metadata={"relevance": "low"})
